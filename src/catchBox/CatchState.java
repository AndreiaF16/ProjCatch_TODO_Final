package catchBox;

import agentSearch.Action;
import agentSearch.State;


import java.awt.*;
import java.util.ArrayList;
import java.util.Arrays;

import static java.lang.Math.abs;

public class CatchState extends State implements Cloneable {
    //TODO this class might require the definition of additional methods and/or attributes

    protected int[][] matrix;
    private int lineCatch;
    private int columnCatch;



    public CatchState(int[][] matrix) {
        //DONE
        //o valor que esta na posição vazia é 0

        this.matrix = new int[matrix.length][matrix.length];

        for (int i = 0; i < matrix.length; i++) {
            for (int j = 0; j < matrix.length; j++) {
                this.matrix[i][j] = matrix[i][j];
                if (this.matrix[i][j] == Properties.CATCH) {
                    lineCatch = i;
                    columnCatch = j;
                }
            }
        }
    }

    public void executeAction(Action action) {
        action.execute(this);
        // DONE
        fireUpdatedEnvironment();

    }

    public boolean canMoveUp() {
        //DONE
        if(lineCatch == 0){
            return false;
        }
        if(matrix[lineCatch-1][columnCatch] == Properties.WALL){
            return false;
        }
        return true;

    }

    public boolean canMoveRight() {
        //DONE
        if(columnCatch == matrix.length-1){
            return false;
        }
        if(matrix[lineCatch][columnCatch+1] == Properties.WALL){
            return false;
        }
        return true;
    }

    public boolean canMoveDown() {
        //DONE

        if(lineCatch == matrix.length-1){
            return false;
        }
        if(matrix[lineCatch+1][columnCatch] == Properties.WALL){
            return false;
        }
        return true;
    }

    public boolean canMoveLeft() {
        //DONE
        if(columnCatch == 0){
            return false;
        }
        if(matrix[lineCatch][columnCatch-1] == Properties.WALL){
            return false;
        }
        return true;
    }

    public void moveUp() {
        //DONE
        //++ na linha pq mexe na linha, a coluna é sempre a mesma
        matrix[lineCatch][columnCatch]= Properties.EMPTY;
        matrix[--lineCatch][columnCatch]= Properties.CATCH;

    }

    public void moveRight() {
        //DONE
        //++ na coluna pq mexe na coluna, a linha é sempre a mesma
        matrix[lineCatch][columnCatch]= Properties.EMPTY;
        matrix[lineCatch][++columnCatch]= Properties.CATCH;
    }

    public void moveDown() {
        //DONE
        //-- na linha pq mexe na linha, a coluna é sempre a mesma
        matrix[lineCatch][columnCatch]= Properties.EMPTY;
        matrix[++lineCatch][columnCatch]= Properties.CATCH;
    }

    public void moveLeft() {
        //DONE
        //-- na coluna pq mexe na coluna, a linha é sempre a mesma
        matrix[lineCatch][columnCatch]= Properties.EMPTY;
        matrix[lineCatch][--columnCatch]= Properties.CATCH;
    }

    public int getNumBox() {
        //DONE ???
        int count=0;
        for (int i = 0; i <matrix.length ; i++) {
            for (int j = 0; j <matrix[0].length; j++) {
                if(matrix[i][j]==Properties.BOX)
                    count++;
            }
        }
        return count;
       // throw new IllegalArgumentException();
    }

    public void setCellCatch(int line, int column) {
        //DONE
        matrix[lineCatch][columnCatch] = Properties.EMPTY;
        lineCatch = line;
        columnCatch = column;
        matrix[lineCatch][columnCatch] = Properties.CATCH;
    }

    public int[][] getMatrix() {

        return matrix;
    }



    public int getSteps() {
        //DONE ?????
        return 0;
    }

    public int getSize()
    {
        return matrix.length;
    }

    public Color getCellColor(int line, int column) {
        switch (matrix[line][column]) {
            case Properties.BOX:
                return Properties.COLORBOX;
            case Properties.CATCH:
                return Properties.COLORCATCH;
            case Properties.DOOR:
                return Properties.COLORDOOR;
            case Properties.WALL:
                return Properties.COLORWALL;
            default:
                return Properties.COLOREMPTY;
        }
    }

    @Override
    public boolean equals(Object other) {
        if (!(other instanceof CatchState)) {
            return false;
        }

        CatchState o = (CatchState) other;
        if (matrix.length != o.matrix.length) {
            return false;
        }

        return Arrays.deepEquals(matrix, o.matrix);
    }

    @Override
    public int hashCode() {
        return 97 * 7 + Arrays.deepHashCode(this.matrix);
    }

    @Override
    public String toString() {
        StringBuilder buffer = new StringBuilder();
        buffer.append(matrix.length);
        for (int i = 0; i < matrix.length; i++) {
            buffer.append('\n');
            for (int j = 0; j < matrix.length; j++) {
                buffer.append(matrix[i][j]);
                buffer.append(' ');
            }
        }
        return buffer.toString();
    }

    @Override
    public CatchState clone() {
        //TODO ???
        //throw new IllegalArgumentException();
         return new CatchState(matrix);

    }

    //Listeners
    private final ArrayList<EnvironmentListener> listeners = new ArrayList<>();

    public synchronized void addEnvironmentListener(EnvironmentListener l) {
        if (!listeners.contains(l)) {
            listeners.add(l);
        }
    }

    public synchronized void removeEnvironmentListener(EnvironmentListener l) {
        listeners.remove(l);
    }

    public void fireUpdatedEnvironment() {
        for (EnvironmentListener listener : listeners) {
            listener.environmentUpdated();
        }
    }



    public int getColumnCatch() {
        return columnCatch;
    }



    public int getLineCatch() {
        return lineCatch;
    }



    public double distancia(int lineGoal, int columnGoal) {
       // return Math.abs(this.getLineGoal() - lineCatch) + Math.abs(this.getColumnGoal() - columnCatch);
        return Math.abs(this.getLineCatch() - lineGoal )+ Math.abs(this.getColumnCatch() - columnGoal);
    }
}
